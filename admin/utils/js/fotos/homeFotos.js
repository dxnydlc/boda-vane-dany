...
const columnasVisibles  = [ "id" , "Tipo" , "Boda" , "Nombre" , "Extension" , "Foto" , "UsuarioMod" , "Estado" , "created_at" ];

// campos que tendrá el formulario
const formFields        = [ "Tipo" , "IdBoda" , "Fecha" , "Hora" , "MapaLink" , "Direccion" , "Estado" ];
...
(function($){
    ...
    $.fn.fileUploader = function (options) {
        ...
        return this.each(function () {
            ...
            // Guarda la instancia en el input
            $input.data("fileUploader", instance);
	...
	$(document).ready(function()
    {
        /* ------------------------------------------------------------- */
        /* ------------------------------------------------------------- */
        const mdlArchivos = new bootstrap.Modal(document.getElementById('mdlArchivos'), {
            keyboard : false
        });
        
        /* ------------------------------------------------------------- */
        /* ------------------------------------------------------------- */
        $("#archivoJML").fileUploader({
            endpoint        : `${urlFotos}upload`,
            token           : tokenBackend , 
            clearPreviewAfterUpload: false , 
            allowedMime: [
                "image/jpeg",
            ...
            extraData: function() {
                return {
                    IdBoda       : $('#frmFoto #IdBoda').val() , 
                    Tipo         : $('#frmFoto #Tipo').val() , 
                };
            },
        ...
        /* ------------------------------------------------------------- */
        /* ------------------------------------------------------------- */
        $('#mdlArchivos').on('hidden.bs.modal', function () {
            uploader = $("#archivoJML").data("fileUploader");
            uploader.clearPreview();
        });
        /* ------------------------------------------------------------- */
        /* ------------------------------------------------------------- */
        /* ------------------------------------------------------------- */
        ...
        /* ------------------------------------------------------------- */
        /* ------------------------------------------------------------- */
        $("#btnmAbrirModal").on( "click", function(e) {
            e.preventDefault();
            abrirModalArchivos();
        });
        /* ------------------------------------------------------------- */
...
function prepararRequest( tipoReq ) {
    ...
    switch ( tipoReq ) {
        ...
        case 'guardar-cab':
            ...
            //idCab           = id;
            //uuidCab         = uu_id;
            xUrl            = `${urlFotos}guardar`;
            xMetodo         = `POST`;

            if( idCab > 0 ){
                xUrl            = `${urlFotos}actualizar/${uu_id}`;
                xMetodo         = `PATCH`;
            }
        break;
        // -------------------------------------------------------------
        case 'listar-cab':
            xUrl             = `${urlFotos}get-activos`, 
            xMetodo          = `GET`;
        break;
        // -------------------------------------------------------------
        case 'cargar-cab':
            xUrl             = `${urlFotos}get-by-id/${idCab}`, 
            xMetodo          = `GET`;
        break;
        // -------------------------------------------------------------
        case 'anular-cab':
            xUrl             = `${urlFotos}anular-by-id/${idCab}`;
            xMetodo          = `DELETE`;
        break;
        // -------------------------------------------------------------
        case 'get-bodas':
    ...
    if (status === 200) {
        ...
        switch ( tipoReq ) {
            ...
            case 'get-bodas':
                ...
                llenarCombo( arrBodas , `#frmFoto #IdBoda` , true );
            break;
            // -------------------------------------------------------------
            // -------------------------------------------------------------
            // -------------------------------------------------------------
...
function abrirModalArchivos() {
    const modalEl = document.getElementById('mdlArchivos');
...
/* ------------------------------------------------------------- */
// Cerrar modal
document.getElementById("fxModal").addEventListener("click", () => {
    const modal = document.getElementById("fxModal");
    const img   = document.getElementById("fxImg");
